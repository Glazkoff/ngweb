...
function portfolio_post_type_messages($messages)
{
	...
	$messages["portfolio"] = [
		// portfolio - название созданного нами типа записей
		0 => "", // Данный индекс не используется.
		1 => sprintf(
			'Работа портфолио обновлена. <a href="%s">Просмотр</a>',
		...
		5 => isset($_GET["revision"])
			? sprintf(
				"Работа портфолио восстановлена из редакции: %s",
				wp_post_revision_title((int) $_GET["revision"], false)
			)
			: false,
		6 => sprintf(
			'Работа портфолио опубликована на сайте. <a href="%s">Просмотр</a>',
			esc_url(get_permalink($post_ID))
		),
		7 => "Услуга сохранена.",
		8 => sprintf(
			'Отправлено на проверку. <a target="_blank" href="%s">Просмотр</a>',
			esc_url(add_query_arg("preview", "true", get_permalink($post_ID)))
		),
		9 => sprintf(
			'Запланировано на публикацию: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Просмотр</a>',
			date_i18n(__("M j, Y @ G:i"), strtotime($post->post_date)),
			esc_url(get_permalink($post_ID))
		),
		10 => sprintf(
			'Черновик обновлён. <a target="_blank" href="%s">Просмотр</a>',
			esc_url(add_query_arg("preview", "true", get_permalink($post_ID)))
		),
	];
...
function portfolio_post_type_help_tab()
{
	...
	$args = [
		...
		"content" =>
			"<h3>Доступные действия с типом постов «" .
			$screen->post_type .
			"»</h3><p>Содержимое второй вкладки</p>",
	];
